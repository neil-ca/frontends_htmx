...
public class SocketHandler extends TextWebSocketHandler {
    ...
    public void handleTextMessage(WebSocketSession session, TextMessage message)
            throws IOException {
        Map<String, Object> value = objectMapper.readValue(message.getPayload(), new com.fasterxml.jackson.core.type.TypeReference<>() {
            ...
            try {
                ...
                if (webSocketSession.equals(currentSession)) {
                    webSocketSession.sendMessage(
                            new TextMessage(userMessageHtml(message)));
                } else {
                    webSocketSession.sendMessage(
                            new TextMessage(incomingMessageHtml(message)));
                }
            } catch (IOException e) {
    ...
    private String userMessageHtml(String message) {
        ...
        return templateEngine.process("websockets", Set.of("user-message"), context);
    }

    private String incomingMessageHtml(String message) {
        Context context = new Context(null, Map.of("message", message));
        return templateEngine.process("websockets", Set.of("incoming-message"), context);
    }
}
